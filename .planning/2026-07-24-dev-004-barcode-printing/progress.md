...
- User authorized submission and publication. Commit `82675a1` was pushed to `codex/dev-004-barcode-printing`, and PR #4 was created at https://github.com/garyyue2019/OpenLIMS/pull/4.
- The first Windows CI run failed because its preinstalled PostgreSQL 14 service was disabled/stopped and `OPENLIMS_TEST_POSTGRES_CONNECTION` was absent; all six Labeling integration tests failed closed before setup rather than failing business assertions.
- The Windows job now starts the documented `postgresql-x64-14` service, verifies readiness, and supplies the synthetic `postgres/root` test connection. The Labeling task profile and all mandatory specification gates were rerun locally and passed before publishing the fix.
