...
- The existing Result contract already distinguishes `INITIAL`, `DUPLICATE`, `RETEST`, `SUPPLEMENT`, `RE_PREPARATION`, and `RE_SAMPLING` observations, stores trigger reasons/approval references, records adoption rules before adoption, and enforces one effective adoption through an append-only version chain. Phase 7C should extend this model rather than create a separate retest aggregate.
- The confirmed Result gaps are deterministic calculation execution and explicit accreditation eligibility evidence at execution/result time; retest completion needs focused verification and only narrow strengthening if the existing domain does not already require predeclared rules.
- Phase 7C confirmed the Result public contract lives under `contracts/result`, while the existing published migration `20260726_001_result_adoption` owns only observations, derivations, adoption rules, and adoption versions. Calculation execution and accreditation evidence therefore require additive contract types and a new migration; the published migration SQL must remain unchanged.
- The existing Result mutation pipeline already supplies group-level locking, exact version checks, object authorization, transactional audit/outbox writes, and fail-closed attempt evidence. Phase 7C should add calculation and accreditation commands through that pipeline and model eligibility queries after the existing conclusion-evidence port rather than create a parallel control path.
- PRD requirements `LAB-CALC-001` and `LAB-ACC-001` require deterministic versioned calculation for unit conversion, dilution, rounding, LOD/LOQ, and limit comparison, plus accreditation checks bound to laboratory/site, method version, product or matrix, parameter/range, validity period, and authorized personnel. The PRD does not supply universal production defaults, so callers must provide explicit versioned rules/evidence and unsupported units or modes must fail closed.
- Result aggregate concurrency is derived from the maximum `group_version` across every append-only fact table. New calculation executions and accreditation assessments must share that version sequence, enforce one fact per group version, participate in aggregate loading, and emit the same transactional platform audit/outbox evidence as existing observations and adoptions.
- Phase 7C design uses an existing observation/derivation/calculation target plus an explicit versioned calculation-rule snapshot containing source/output units, conversion, dilution/quantity factors, rounding, LOD/LOQ, and limit comparison. Accreditation is an append-only two-stage model: `EXECUTION` validates site/method/product/parameter/person/validity, while `RESULT` additionally binds the current effective adoption target and accredited numeric range; eligibility requires compatible passing evidence at both stages.
- Accreditation eligibility must also authorize the current querying/signing actor against the latest result-stage personnel list; checking only the actor who recorded the assessment would allow another scoped but unqualified user to reuse that evidence.
- Solution-level locked restore revealed that Phase 7B had not refreshed every Host-dependent lock file after Operations registration. A solution `--force-evaluate` followed by successful `--locked-mode` restore repaired the mechanical dependency graph across API/Worker contract consumers.
- Phase 7D will keep downstream state in the owning modules. Report delivery is modeled as append-only version-bound delivery records, hashed download grants bound to one recipient and expiry, and notification attempts whose status is derived from immutable attempts. A historical token can return only its stored report/version/content hash or fail closed; it can never resolve through a current-version alias.
- Report recipient access requires the token recipient to match the authenticated actor plus exact `report.receive` object-scope authorization. Delivery administration remains under `report.manage`. Notification success requires an external reference, while failed and unknown attempts remain queryable.
- Billing integration is modeled as immutable export batches containing copied evidence/adjustment totals, then append-only ERP or invoice handoffs and idempotent attempts. `SUCCEEDED` requires an external reference; ERP success additionally requires voucher/company/fiscal/period/posting evidence. `FAILED`, `UNKNOWN`, and `DIFFERENT` remain in the difference queue without reverting billing facts.
- External providers are not invoked or simulated in Phase 7D. The runtime supplies durable automated/manual handoff boundaries, retry idempotency, status queries, audit/outbox evidence, and caller-recorded external acknowledgements.
- Phase 7E will add an `ai` runtime module around the existing `OpenLIMS.Contracts.Ai` types. Run requests, provider outcomes, and human dispositions are separate append-only facts so a provider call never holds a database transaction and a lost response can retry with the same run ID as the provider idempotency key.
- The default AI provider adapter is disabled. It records `PROVIDER_DISABLED` plus `ManualFallbackRequired=true`; no built-in adapter can claim a model succeeded. Deployments may replace the public provider port, while tests use deterministic fakes.
- Provider requests expose only exact model/prompt/schema/input references, route, and validation profile; no arbitrary raw document body is passed through the core port. Runtime output must semantically match the requested envelope. Envelope drift, invalid provider confirmations, unknown fields/units/sources, or prohibited fact promotion retain the original output but produce a quarantined outcome with no downstream candidates.
- Human dispositions are append-only, expected-version checked, idempotent, and built server-side with the authenticated actor plus the candidate's original AI value. They never alter the candidate fact class or write controlled Commercial/Knowledge data automatically.
- The resumed 7E worktree contains the extended AI public contract plus seven untracked module files: descriptor/DI, authorization, disabled provider, runtime rules, migration, persistence, and project metadata. Service orchestration, HTTP endpoints, telemetry, host registration, solution registration, and runtime tests are still absent.
- The AI skeleton already follows the established module boundary: exact-scope claims, disabled-by-default provider, append-only request/outcome/disposition/attempt tables, provider calls intended outside transactions, and runtime validation that accepts only exact envelopes and quarantines invalid or promoted facts. The migration limits outcomes to accepted/quarantined/disabled/failed, so pending is a derived query state before an outcome exists.
- The public runtime contract exposes create/get/disposition/review-queue operations, exact `ai.run` and `ai.review` capabilities, a versioned provider port, and a result that preserves the original output plus every human disposition. Persistence derives run version as request version 1 plus disposition count; the outcome itself is terminal execution evidence and does not increment the human-review version chain.
- `AiStore` already has all required transaction-bound primitives: idempotency/request locks, request and outcome inserts with platform audit/outbox, result reconstruction, disposition idempotency/version storage, queue IDs, and read-audit intents. The service must wrap each database phase in `IPostgresTransactionCoordinator`, recheck terminal state after locking, and call the provider only between transactions.
- Existing modules name the shared transaction abstraction `ITransactionCoordinator` and use `ICurrentOrganizationContext`, `ICurrentActorContext`, `IClock`, `IIdGenerator`, module authorization, a fail-closed standalone attempt writer, and module telemetry. AI endpoints should reuse the same JSON/problem/correlation conventions and map version/idempotency conflicts to 409, inaccessible runs to 404, quarantined/review-invalid states to 422, authorization to 403, and persistence failure to 503.
- Existing AI validation already enforces stable envelope references, allowed fields/units, source locations, duplicate determinate rejection, fact-class promotion evidence, valid gap suggestions, and immutable original values in human dispositions. Runtime tests can focus on orchestration and storage rather than duplicating every contract-only validation case.
- AI runtime test projects should mirror the established three-layer layout: unit project references module+contract, HTTP contract project references module+contract+API host, and PostgreSQL integration project references module+contract+platform building blocks. All require solution registration and generated lock files.
- Host composition is explicit in both API and Worker: each needs an AI namespace import, module instance, project reference, and solution entry. API telemetry uses an explicit meter allowlist, and the host serves a hand-maintained OpenAPI path dictionary, so the four AI paths must be registered there as well as in endpoint mapping.
- Integration tests can use the standard platform dependency registration against a dedicated PostgreSQL database, replace AI authorization/provider ports with deterministic fakes, and assert platform audit/outbox plus module attempt tables directly. HTTP contract tests should replace `IAiRunService` with a stub, use the existing test authentication pattern, verify stable problem mappings, and assert all four OpenAPI operation IDs.
- The AI contract project is already registered in `OpenLIMS.slnx`; only the runtime module and new unit/integration projects need new solution folders. The existing AI contract-test project can be expanded in place to reference the API host and runtime module, avoiding a duplicate HTTP contract project.
- Focused AI verification is green after boundary hardening: unit 8/8, combined contract+HTTP 19/19, and PostgreSQL integration 8/8. The worktree is whitespace-clean; generated lock files now exist for the runtime, unit, and integration projects, while API/AI-contract locks reflect the new transitive module graph.
- Phase 7E full verification also passes solution `--locked-mode` restore, solution Release build with 0 warnings/errors, the complete .NET test matrix including architecture 19/19, and repository engineering contracts 9/9. AI remains optional and disabled by default; external provider exceptions and invalid confirmations persist terminal manual-fallback outcomes rather than fabricating success or leaving permanent pending state.
