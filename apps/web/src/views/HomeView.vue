...
<template>
  <main class="hero" aria-labelledby="page-title">
    <a-card :bordered="false" class="hero-card">
      ...
      <p>通过受保护的工作台执行从商业受理、收样与检测，到受控报告交付和外部交接的实验室全流程操作。</p>
      ...
      <section v-if="authSnapshot.status === 'authenticated'" class="operator-launchpad" aria-labelledby="operator-launchpad-title">
        ...
        <div class="operator-grid">
          <RouterLink :to="{ name: 'workbench.commercial' }"><strong>商业受理</strong><span>处理询价缺口、能力复核和报价版本</span></RouterLink>
          ...
          <RouterLink :to="{ name: 'workbench.operations' }"><strong>样品作业</strong><span>维护谱系、监管链、计划、资源和队列</span></RouterLink>
          <RouterLink :to="{ name: 'workbench.instrument' }"><strong>仪器导入</strong><span>登记文件、解析行并处理异常</span></RouterLink>
          <RouterLink :to="{ name: 'workbench.result' }"><strong>结果完成</strong><span>维护来源、计算、认可、规则与采用版本</span></RouterLink>
          <RouterLink :to="{ name: 'workbench.qc' }"><strong>QC 放行</strong><span>传播影响并满足五个放行门</span></RouterLink>
          <RouterLink :to="{ name: 'workbench.report' }"><strong>报告交付</strong><span>评估门禁、签发、交付并验证版本链</span></RouterLink>
          <RouterLink :to="{ name: 'workbench.billing' }"><strong>计费交接</strong><span>创建证据、导出批次并跟踪外部差异</span></RouterLink>
          <RouterLink :to="{ name: 'workbench.ai-review' }"><strong>AI 复核</strong><span>查看隔离输出并记录人工处置</span></RouterLink>
        </div>
      </section>
    </a-card>
  </main>
