...
public sealed partial class ArchitectureBoundaryTests
{
    ...
    public void Module_public_contracts_do_not_expose_private_persistence_types_or_tables()
    {
        var contractRoots = new[]
        {
            ...
            Path.Combine(RepositoryRoot, "contracts", "instrument"),
            Path.Combine(RepositoryRoot, "contracts", "qc"),
            Path.Combine(RepositoryRoot, "contracts", "report"),
            Path.Combine(RepositoryRoot, "contracts", "toy"),
    ...
    public void Production_hosts_use_an_explicit_receiving_module_manifest()
    {
        ...
        Assert.All(hostPrograms, program => Assert.Contains("new BillingModule(", File.ReadAllText(program), StringComparison.Ordinal));
        ...
        Assert.All(hostPrograms, program => Assert.Contains("new ToyModule(", File.ReadAllText(program), StringComparison.Ordinal));
    ...
    [Fact]
    public void Billing_persistence_accesses_only_its_private_schema()
    {
        var moduleRoot = Path.Combine(RepositoryRoot, "src", "modules", "billing");
        var sql = Directory.EnumerateFiles(moduleRoot, "*.cs", SearchOption.AllDirectories)
            .SelectMany(file => SchemaAccessPattern().Matches(File.ReadAllText(file)))
            .Select(match => match.Groups[1].Value)
            .ToArray();
    ...
    [Fact]
    public void Instrument_persistence_accesses_only_its_private_schema()
    {
        var moduleRoot = Path.Combine(RepositoryRoot, "src", "modules", "instrument");
        var sql = Directory.EnumerateFiles(moduleRoot, "*.cs", SearchOption.AllDirectories)
            .SelectMany(file => SchemaAccessPattern().Matches(File.ReadAllText(file)))
            .Select(match => match.Groups[1].Value)
            .ToArray();
        ...
        Assert.NotEmpty(sql);
        Assert.All(sql, schema => Assert.Equal("instrument", schema, ignoreCase: true));
    }

    [Fact]
    public void Qc_persistence_accesses_only_its_private_schema()
    {
        var moduleRoot = Path.Combine(RepositoryRoot, "src", "modules", "qc");
        var sql = Directory.EnumerateFiles(moduleRoot, "*.cs", SearchOption.AllDirectories)
            .SelectMany(file => SchemaAccessPattern().Matches(File.ReadAllText(file)))
            .Select(match => match.Groups[1].Value)
            .ToArray();
        ...
        Assert.NotEmpty(sql);
        Assert.All(sql, schema => Assert.Equal("qc", schema, ignoreCase: true));
    }

    [Fact]
    public void Report_persistence_accesses_only_its_private_schema()
    {
        var moduleRoot = Path.Combine(RepositoryRoot, "src", "modules", "report");
        var sql = Directory.EnumerateFiles(moduleRoot, "*.cs", SearchOption.AllDirectories)
            .SelectMany(file => SchemaAccessPattern().Matches(File.ReadAllText(file)))
            .Select(match => match.Groups[1].Value)
            .ToArray();
        ...
        Assert.NotEmpty(sql);
        Assert.All(sql, schema => Assert.Equal("report", schema, ignoreCase: true));
    }
